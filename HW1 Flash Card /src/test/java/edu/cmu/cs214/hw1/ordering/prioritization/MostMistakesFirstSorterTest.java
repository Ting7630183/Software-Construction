...
public class MostMistakesFirstSorterTest {
    CardOrganizer mostMistakesFirstSorter = new MostMistakesFirstSorter();
    ...
    public void testReorganize(){
        ...
        status3.recordResult(true);
        List<CardStatus> sorted = mostMistakesFirstSorter.reorganize(cards);
        assertEquals(sorted.get(0), status1);
        assertEquals(sorted.get(1), status2);
        assertEquals(sorted.get(2), status3);
    }
}
